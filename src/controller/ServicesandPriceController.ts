...
} from "../services/ServicesandPriceService.ts";
  ...
  try {
    ...
    if (!payload.id || !payload.merchantId || !payload.serviceId) {
      return res.status(400).json({
        status: "error",
        statusCode: 400,
        message: "id, merchantId, and serviceId are required",
      });
    }
    const merchantService =
      await servicesandPriceService.createMerchantService(payload);
...
export const getMerchantServices = async (
  ...
  try {
    ...
    const merchantServices =
      await servicesandPriceService.getMerchantServices(merchantId);
...
export const createPricingRule = async (
  ...
  try {
    ...
    //     status: "error",
    //     statusCode: 400,
    //     message: "id, merchantServiceId, price, and attributes are required",
    //   });
    // }
    const pricingRule =
      await servicesandPriceService.createPricingRule(payload);
  ...
  try {
    ...
    if (!merchantServiceId) {
      return res.status(400).json({
        status: "error",
        statusCode: 400,
        message: "merchantServiceId is required",
      });
    }
    const pricingRules =
      await servicesandPriceService.getPricingRules(merchantServiceId);
  ...
  try {
    ...
    if (!payload.id || !payload.name) {
      return res.status(400).json({
        status: "error",
        statusCode: 400,
        message: "Attribute id and name are required",
      });
    }
...
export const updatePricingRuleController = async (
  ...
  try {
    const { id, price, attributes } = req.body;

    if (!id) {
      return res.status(400).json({ message: "ID is required" });
    }

    const updatedRule = await servicesandPriceService.updatePricingRule({
      id,
      price,
      attributes,
    });
  ...
  try {
    ...
    if (!payload.id || !payload.attributeId || !payload.value) {
      return res.status(400).json({
        status: "error",
        statusCode: 400,
        message: "id, attributeId, and value are required",
      });
    }
    const attributeValue =
      await servicesandPriceService.createAttributeValue(payload);
